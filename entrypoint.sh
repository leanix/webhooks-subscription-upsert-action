...
NEW_SUBSCRIPTION_FILE=/github/workspace/${INPUT_SUBSCRIPTION_FILE}
echo "Reading provided subscription definition from file '${NEW_SUBSCRIPTION_FILE}' ..."

SUBSCRIPTION_IDENTIFIER="$(cat ${NEW_SUBSCRIPTION_FILE} | jq -r '.identifier')"
echo "Found provided subscription with identifier ='${SUBSCRIPTION_IDENTIFIER}'"
...
for REGION in $REGIONS; do
  ...
  if [ "${SUBSCRIPTION_ID}" != "null" -a ! -z "${SUBSCRIPTION_ID}" ] ; then
    ...
    if [[ "${UPSERT_RESULT}" -eq 200 ]] ; then
      ...
      echo "Failed to update subscription. id='${SUBSCRIPTION_ID}' http-code='${UPSERT_RESULT}'"
      exit 1
    fi
  else
    ...
    if [[ "${CREATE_RESULT}" -eq 200 ]] ; then
      ...
      echo "Failed to create new subscription. http-code='${CREATE_RESULT}'"
      exit 1
    fi
  fi
done