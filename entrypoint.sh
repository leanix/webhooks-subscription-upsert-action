...
echo "Login to Azure ..."
#CCB az login --service-principal -u $ARM_CLIENT_ID -p $ARM_CLIENT_SECRET --tenant $ARM_TENANT_ID
#CCB az account set -s $ARM_SUBSCRIPTION_ID
...
for REGION in $REGIONS; do
  ...
  #CCB VAULT_SECRET_VALUE=$(az keyvault secret show --vault-name ${KEY_VAULT_NAME} --name ${VAULT_SECRET_KEY} | jq -r .value)
  VAULT_SECRET_VALUE="rBwqqfTNGx0MQWJXVQFD"
